...
def log_alert(symbol: str, data: dict, score_result: dict) -> None:
    ...
    _write_csv("data/alert_log.csv", row, fields)
    ...
    if config.ENABLE_PUMP_TRACKING:
        ...
        _write_csv("data/pump_tracker.csv", pump_row,
                   ["alert_time","symbol","alert_price","score","check_date",
                    "price_at_check","pump_pct","pumped"])


def update_pump_tracker(prices: dict) -> None:
    if not config.ENABLE_PUMP_TRACKING or not os.path.isfile("data/pump_tracker.csv"):
        return
    today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    rows  = []
    updated = 0
    with open("data/pump_tracker.csv", "r", newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            rows.append(row)
    if updated > 0:
        with open("data/pump_tracker.csv", "w", newline="", encoding="utf-8") as f:
...
def get_accuracy_stats() -> dict:
    if not os.path.isfile("data/pump_tracker.csv"):
        return {}
    with open("data/pump_tracker.csv", "r", newline="", encoding="utf-8") as f:
